...
resource "aws_instance" "redacre" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "pip install docker",
      "sudo docker network create -d bridge redacre",
      "sudo docker run -d --net=redacre --restart=always --name backend kurtay/backend:latest",
      "sudo docker run -d -p 80:80 --net=redacre --restart=always --name frontend kurtay/frontend:latest",
    ]
  }
}
